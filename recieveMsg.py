...
commands = {"ADD": 1,
            ...
            "CLEAR": 4,
            "SHOW": 5}
# provide path to your excel file, and the sheet name 
path = None
sheetName = None
birthdays = pd.read_excel(f"{path}", sheet_name=f"{sheetName}") # Used to update the excel sheet holding data

# clears all records
def clear():
    global birthdays
    birthdays.drop(birthdays.index, inplace=True)
    with pd.ExcelWriter(f"{path}", engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
            birthdays.to_excel(writer, sheet_name=f"{sheetName}",index=False)
...
def whatsapp_reply():
    ...
    "4.Read documentation for all the information.\n"
        ...
        response = MessagingResponse()
        response.message("Enter birthdates to be reminded!")
        return str(response)